    ...
    def maintain_loop():
        ...
        while self.alive:
            try:
                ...
                try:
                    # 获取 sd 完成的任务
                    sd_done_task_res = requests.request(
                        method="GET",
                        url="{}/api/v1/robot/getDoneWxSdTask".format(self.config.get("server")),
                        headers={
                            'Accept': 'application/json',
                        },
                        json={
                            "wx_bot": self.storageClass.userName,
                            "ib_username": self.config.get("ib_username")
                        },
                    )
                    print(sd_done_task_res.json())

                    for done in sd_done_task_res.json()["data"]:
                        # 发完成消息
                        self.send(
                            "{}@{} \n✅任务已经完成\n🌟任务 ID：{}\n✨作品已自动存储到图库中\n🔗图片链接：{}\n{}".format(
                                done["head_msg"],
                    ...
                    # 获取 mj 完成的任务
                    mj_done_task_res = requests.request(
                        method="GET",
                        url="{}/api/v1/robot/getDoneWxMjTask".format(self.config.get("server")),
                        headers={
                            'Accept': 'application/json',
                        },
                        json={
                    ...
                    print(mj_done_task_res.json())
                    for done in mj_done_task_res.json()["data"]:
                        # 发完成消息
                        self.send(
                            "{}@{} \n✅任务已经完成\n🌟任务 ID：{}\n✨作品已自动存储到图库中\n🔗图片链接：{}\n{}".format(
                                done["head_msg"],
